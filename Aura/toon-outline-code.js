...
// correctly tracks any rotation/pose of the model — unlike flat canvas
// billboards. See toon-outline-apply.js for how the shell material/meshes
// actually get created, and index.html for how they're attached to the
// loaded model and synced each frame.
import * as THREE from 'three';

export let modelGroundY = 0;
export let TARGET_HEIGHT = 1;
// modelGroundY/TARGET_HEIGHT are ES-module live bindings — importers get a
// read-only view, so the host (index.html) must call these setters instead
// of assigning the imported names directly.
export function setModelGroundY(v) { modelGroundY = v; }
export function setTargetHeight(v) { TARGET_HEIGHT = v; }

// Every hugging-aura layer's tuned constants (SHELL.pushAmount, .noiseAmp,
// ...) are absolute world-space numbers, tuned assuming the character
// renders at this height — see README-INTEGRATION.md's "Scale mismatch"
// section. The host normalizes the loaded model's own scale to match this
// exactly (rather than hand-rescaling each layer's amplitude/frequency
// fields), so every constant below applies unchanged regardless of the
// source model's native GLB scale.
export const SHELL_REFERENCE_HEIGHT = 6.6;
...
export const SHELL = {
    ...
    dissolveFreq: 0.5,
    dissolveSpeed: 0.5,
    // A per-reload/reroll offset baked into the sample coordinate — a differently-
    // arranged (but equally coherent) pattern without touching any other field.
    randomSeed: 0,
    ...
    // Periodic spike system (same mechanism as CONFIG.layer1) is present here only
    // because the shader is shared with other hugging-aura layers; periodicSpikeAmp
    // stays at 0 so it has zero effect and the Toon Outline's look/behavior is
    // unchanged.
    ...
    // Remaining layer1-parity fields — all zero/neutral so they have no effect here.
    subSpikes: 0,
    subSpikeLength: 0,
...
export const SHELL_FLOW_MODES = { left: 0, right: 1, converge: 2 };

// Shared shader source for any "mesh-hugging" material — the toon outline and any
// additional hugging aura layers all use this same vertex/fragment logic, just
// with their own independent uniforms objects and their own pushAmount/noise
// scale, so a thin outline and a thick flame shell are really the same technique
// at different settings.
export const HUGGING_AURA_VERTEX_SHADER = `
            ...
            // shader is used for the Toon Outline, so that material's look is unchanged.
            ...
            // no-op values (0, or 1 where a multiplier) for the Toon Outline so its look
        ...
        `;
export const HUGGING_AURA_FRAGMENT_SHADER = `
            uniform vec3 uColor;
...
export function buildHuggingAuraUniforms(cfg) {
    return {
...
export function syncHuggingAuraUniforms(cfg, uniforms, time) {
